#include "BlessedKingdom_3.h"

BlessedKingdom_3::BlessedKingdom_3(int line, bool isOwned, int unitId, int playerId)
{
...
BlessedKingdom_3::~BlessedKingdom_3()
{
}

void BlessedKingdom_3::Die()
{
...
void BlessedKingdom_3::Attack(vector<BaseUnitClass*>& targets)
{
	this->action = "Attack";
...
vector<BaseUnitClass*> BlessedKingdom_3::FindTargets()
{
	...
	for (auto target : BaseUnitClass::AllIngameUnit_Vector) {
		...
		if (this->isOwned == target->isOwned
			...
			if (!isAffected)
				target->ApplyStatus(StatusReceive("Blessing Aura", "Regeneration", 3.5*target->maxHealth / 100.0, Tool::currentIngameTime + 2, 1));
		}
	}
