#include "ElementalAlien_4.h"

ElementalAlien_4::ElementalAlien_4(int line, bool isOwned, int unitId, int playerId)
{
...
ElementalAlien_4::~ElementalAlien_4()
{
}

void ElementalAlien_4::Attack(vector<BaseUnitClass*>& targets)
{
	this->action = "Attack";
	...
	for (auto target : targets) {
		...
		if (this->element == "Ice") {
			damage += target->attack * 0.35;
		}
		else if (this->element == "Fire") {
			damage += target->baseRegeneration * 5;
			bool isAffected = false;
			for (int i = 0; i < target->statusReceive.size(); i++) {
				if (target->statusReceive[i].statusName == "Unhealable") {
					isAffected = true;
					target->statusReceive[i].releaseStatusTime = Tool::currentIngameTime + 4;
					break;
				}
			}
			if(!isAffected)
				target->ApplyStatus(StatusReceive("Unhealable", "Regeneration", 0, Tool::currentIngameTime + 4, 5));
		}
		else if (this->element == "Nature") {
			this->currentHealth += (target->maxHealth *0.06 * (120 / target->moveSpeed));
